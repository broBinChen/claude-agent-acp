...
describe("canUseTool: AskUserQuestion bridge", () => {
    ...
    it("accept → returns allow with answers in updatedInput", async () => {
        ...
        const result = await agent.canUseTool("sess-1")("AskUserQuestion", sampleInput, baseOpts());
        ...
        expect(result.updatedInput.answers).toEqual({ "Which lib?": "A" });
        expect(result.updatedInput.questions).toEqual(sampleInput.questions);
    });
    // -------------------------------------------------------------------
    ...
    it("aborted signal during elicitation throws 'Tool use aborted'", async () => {
        ...
        await expect(agent.canUseTool("sess-1")("AskUserQuestion", sampleInput, {
            ...baseOpts(),
            signal: ctrl.signal,
    ...
    it("non-AskUserQuestion tools fall through to existing requestPermission flow", async () => {
        ...
        const result = await agent.canUseTool("sess-1")("Bash", { command: "ls" }, baseOpts());
    ...
    it("client without elicitation.form capability → deny gracefully", async () => {
        ...
        expect(client.unstable_createElicitation).not.toHaveBeenCalled();
        expect(result.behavior).toBe("deny");
        expect(result.message).toMatch(/elicitation\.form|plain text/i);
    });
    // -------------------------------------------------------------------
    ...
    it("malformed toolInput (no questions array) → deny gracefully", async () => {
        const result = await agent.canUseTool("sess-1")("AskUserQuestion", { foo: "bar" }, baseOpts());
        expect(client.unstable_createElicitation).not.toHaveBeenCalled();
        expect(result.behavior).toBe("deny");
        expect(result.message).toMatch(/questions array|malformed|invalid/i);
    });
    // -------------------------------------------------------------------
    ...
    it("malformed toolInput (questions not array) → deny gracefully", async () => {
        const result = await agent.canUseTool("sess-1")("AskUserQuestion", { questions: "not an array" }, baseOpts());
        expect(client.unstable_createElicitation).not.toHaveBeenCalled();
        expect(result.behavior).toBe("deny");
    });
});
